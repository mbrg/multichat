...
describe('AuthPopup', () => {
  ...
  describe('visibility', () => {
    ...
    it('renders when isOpen is true', () => {
      ...
      expect(screen.getByText(/Sign in with your GitHub or Google account to save and manage your API keys/)).toBeInTheDocument()
    })
  })
  ...
  describe('backdrop interaction', () => {
    it('calls onClose when backdrop is clicked', () => {
      ...
      const backdrop = screen.getByRole('button', { name: /continue without signing in/i }).closest('div')?.previousSibling as HTMLElement
      if (backdrop) {
        fireEvent.click(backdrop)
      }
      
      // Since backdrop click is harder to test directly, let's test the "Continue without signing in" button
      const continueButton = screen.getByRole('button', { name: /continue without signing in/i })
  ...
  describe('authentication buttons', () => {
    it('displays GitHub and Google sign-in buttons', () => {
      render(<AuthPopup isOpen={true} onClose={mockOnClose} />)
