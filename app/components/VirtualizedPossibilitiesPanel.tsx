...
> = ({
  ...
  useEffect(() => {
    if (isActive && messages.length > 0) {
      ...
      const allMetadata = new (require('@/services/ai/PossibilityMetadataService').PossibilityMetadataService)()
        .generatePrioritizedMetadata(settings)
      
      const highPriority = allMetadata
        .filter((m: PossibilityMetadata) => m.priority === 'high' || m.priority === 'medium')
        .slice(0, 12)
      
      highPriority.forEach((meta: PossibilityMetadata) => loadPossibility(meta.id))
    }
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (scrollHeight - scrollTop - clientHeight < 100 && availableMetadata.length > 0) {
  ...
  return (
    <>
        ...
        <div className="px-4 py-2 text-xs text-[#888] flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-[#666]">
              <span className="text-[#4ade80]">✓ {possibilities.filter(p => p.isComplete).length}</span>
              <span className="text-[#fbbf24]">⟳ {possibilities.filter(p => !p.isComplete).length}</span>
              <span className="text-[#6b7280]">⋯ {availableMetadata.length}</span>
            </div>
          </div>
      ...
      >
        {/* Scrollable area */}
        <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-[#2a2a2a] scrollbar-track-transparent" onScroll={handleScroll}>
          ...
          <div className="flex flex-col gap-2 max-w-[1200px] mx-auto">
            {possibilities.map(possibility => {
              ...
              const responseOption: ResponseOption & { systemInstruction?: { name: string } } = {
                id: possibility.id,
                model: {
                ...
                timestamp: new Date(),
                systemInstruction: possibility.metadata.systemInstruction ? { name: possibility.metadata.systemInstruction.name } : undefined
              }
          ...
          {availableMetadata.length > 0 && (
            <div className="text-center py-4">
              ...
              {(() => {
                const providerGroups = availableMetadata.reduce((groups: Record<string, PossibilityMetadata[]>, meta: PossibilityMetadata) => {
                ...
                return (
                  <div className="flex flex-wrap gap-3 justify-center max-w-2xl mx-auto">
                    {Object.entries(providerGroups).map(([provider, metas]) => (
                      <button 
                        key={provider}
                        onClick={() => {
                      ...
                      >
                        ...
                        <span className="text-xs text-[#666]">+{metas.length}</span>
                      </button>
                    ))}
                  </div>
                )
              ...
              <details className="mt-4">
                ...
                <div className="flex flex-wrap gap-2 justify-center max-w-2xl mx-auto mt-3">
                  {availableMetadata.slice(0, 12).map((meta: PossibilityMetadata) => (
                    <button 
                      key={meta.id}
