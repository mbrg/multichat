 ...
 */
async function measureTime<T>(operation: () => Promise<T> | T): Promise<{ result: T; duration: number }> {
...
): Promise<{ averageDuration: number; minDuration: number; maxDuration: number; p95Duration: number }> {
...
describe('Performance Regression Tests', () => {
  ...
  describe('ConnectionPoolService Performance', () => {
    it('should enqueue tasks within performance threshold', async () => {
      ...
      expect(averageDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.CONNECTION_POOL_OPERATION)
    })
  ...
  describe('LoggingService Performance', () => {
    it('should log messages within performance threshold', async () => {
      ...
      expect(averageDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.LOGGING_OPERATION)
    })
  ...
  describe('CircuitBreakerRegistry Performance', () => {
    it('should get breaker within performance threshold', async () => {
      ...
      expect(averageDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.CIRCUIT_BREAKER_OPERATION)
    })
    ...
    it('should execute through breaker within threshold', async () => {
      ...
      expect(averageDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.CIRCUIT_BREAKER_OPERATION * 2) // Allow more time for execution
    })
  })
  ...
  describe('EventBus Performance', () => {
    it('should subscribe to events within performance threshold', async () => {
      ...
      expect(averageDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.EVENT_BUS_OPERATION)
    })
  ...
  describe('Component Initialization Performance', () => {
    it('should initialize ConnectionPoolService within threshold', async () => {
      ...
      expect(duration).toBeLessThan(PERFORMANCE_THRESHOLDS.COMPONENT_INITIALIZATION)
    })
  ...
  describe('Performance Regression Detection', () => {
    it('should detect if operations become significantly slower', async () => {
      ...
      expect(withDelay.averageDuration).toBeGreaterThan(baseline.averageDuration)
      
      // But both should still be under threshold
      expect(baseline.averageDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.LOGGING_OPERATION)
      expect(withDelay.averageDuration).toBeLessThan(PERFORMANCE_THRESHOLDS.LOGGING_OPERATION * 2)
    })
  })
})
