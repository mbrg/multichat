...
export function migrateSettings(
  ...
  if (currentVersion < 1) {
    ...
    if (!migratedSettings.enabledModels || migratedSettings.enabledModels.length === 0) {
      migratedSettings.enabledModels = defaults.enabledModels
    }
    
    // Ensure we have temperature options
    if (!migratedSettings.temperatures || migratedSettings.temperatures.length === 0) {
    ...
    if (migratedSettings.maxInitialPossibilities === undefined) {
      migratedSettings.maxInitialPossibilities = defaults.maxInitialPossibilities
    }
...
export function validateSettings(settings: UserSettings): boolean {
  ...
  if (!settings.enabledModels || settings.enabledModels.length === 0) return false
  ...
  try {
    ...
    const hasEnabledProvider = Object.values(providers).some(enabled => enabled === true)
  ...
  const validTemperatures = settings.temperatures.every(temp => 
