...
describe('SecureStorage Integration Tests', () => {
  ...
  describe('Real Crypto Operations', () => {
    ...
    it('should generate different ciphertext for same plaintext', async () => {
      ...
      expect(await SecureStorage.decryptAndRetrieve(testKey + '1')).toBe(
        testValue
      )
      expect(await SecureStorage.decryptAndRetrieve(testKey + '2')).toBe(
        testValue
      )
    })
  ...
  describe('Large Data Handling', () => {
    it('should handle reasonably large API keys', async () => {
      ...
      const largeKey =
        'sk-' + 'a'.repeat(1000) + 'b'.repeat(1000) + 'c'.repeat(1000)
  ...
  describe('Concurrent Operations', () => {
    it('should handle concurrent encrypt operations', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        expect(await SecureStorage.decryptAndRetrieve(`concurrent-${i}`)).toBe(
          `value-${i}`
        )
      }
    })
