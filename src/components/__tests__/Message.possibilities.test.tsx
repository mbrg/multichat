...
describe('Message - Possibilities', () => {
  ...
  const createMockPossibility = (id: string, content: string, model?: string, probability?: number): MessageType => ({
    id,
    role: 'assistant',
    content,
    timestamp: new Date('2023-01-01T10:00:00Z'),
  ...
  it('renders message without possibilities normally', () => {
    ...
    render(<Message message={message} onSelectPossibility={mockOnSelectPossibility} />)
  ...
  it('handles possibility click', () => {
    const possibility = createMockPossibility('p1', 'Alternative response', 'claude-3', 0.75)
  ...
  it('renders multiple possibilities with correct styling', () => {
    ...
    possibilities.forEach(p => {
      ...
      if (p.probability) {
        expect(screen.getByText(`${Math.round(p.probability * 100)}%`)).toBeInTheDocument()
      }
    })
  })
  ...
  it('applies hover styles to possibilities', () => {
    ...
    const possibilityElement = screen.getByText('Alternative response').closest('.cursor-pointer')
    expect(possibilityElement).toHaveClass('hover:bg-[#1a1a2a]', 'cursor-pointer')
  })

  it('handles possibilities without model or probability', () => {
    const possibilities = [
      createMockPossibility('p1', 'Simple alternative')
    ]
