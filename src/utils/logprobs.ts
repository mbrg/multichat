...
type LogProbData = Array<{
  token: string
  logprob: number
  [key: string]: any // Allow additional fields from different providers
}>
...
export function calculateProbabilityFromLogprobs(
  ...
  try {
    ...
    const logprobValues = logprobs.map(tokenLogprob => {
      ...
      if (typeof tokenLogprob === 'object' && tokenLogprob !== null) {
        ...
        if ('logprob' in tokenLogprob && typeof tokenLogprob.logprob === 'number') {
    ...
    const totalLogprob = logprobValues.reduce((sum, logprob) => sum + logprob, 0)
