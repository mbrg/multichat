...
describe('SecureStorage', () => {
  ...
  describe('Auto-lock Functionality', () => {
    ...
    it.skip('should reset idle timer on activity', async () => {
      vi.useFakeTimers();
      ...
      const mockEncryptedData = btoa('mock-encrypted-data');
      (localStorage.getItem as any).mockReturnValue(mockEncryptedData);
      await SecureStorage.decryptAndRetrieve(testKey);
    ...
    it.skip('should allow manual locking', async () => {
      await SecureStorage.encryptAndStore(testKey, testData);
      expect(SecureStorage.isLocked()).toBe(false);
  ...
  describe('Clear All Functionality', () => {
    it.skip('should clear all data and lock storage', async () => {
      await SecureStorage.encryptAndStore(testKey, testData);
  ...
  describe('Security Features', () => {
    it.skip('should generate non-extractable keys', async () => {
      // Mock no existing key to force generation
      mockStore.get.mockImplementationOnce(() => {
        const request = { 
          onerror: null, 
          onsuccess: null,
    ...
    it.skip('should use random IV for each encryption', async () => {
      await SecureStorage.encryptAndStore(testKey, testData);
      await SecureStorage.encryptAndStore('key2', 'data2');

      // Should call getRandomValues for IV generation
      expect(mockCrypto.getRandomValues).toHaveBeenCalledTimes(2);
    });

    it.skip('should use AES-GCM encryption', async () => {
      await SecureStorage.encryptAndStore(testKey, testData);
